...
def test_ingestion_starts_at_current_bucket_for_quarter_hour_plans(monkeypatch) -> None:
    monkeypatch.setattr(ingestion.dt_util, "now", lambda: datetime(2026, 4, 25, 12, 45, tzinfo=timezone.utc))
    price_state = SimpleNamespace(
        state="1.0",
        attributes={
            "today": [float(index) for index in range(96)],
            "tomorrow": [float(index + 100) for index in range(96)],
        },
    )
    states = {
        "sensor.price": price_state,
    ...
    input_data, status = DataIngestor(hass, config).build_input()

    assert status.ok is True
    assert input_data is not None
    assert input_data.prices[0].start == datetime(2026, 4, 25, 12, 45, tzinfo=timezone.utc)
    assert input_data.prices[0].price == 51.0
